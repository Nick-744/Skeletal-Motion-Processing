...
Pairing that solver with the model-free detector is, as~\autoref{subsec:sota_positioning}
sets out, the specific combination the survey leaves open: the contribution is not a new
algorithm but the assembly of existing components into a system that recovers a
canonical \acrshort{mano} pose at interactive rate on \acrshort{cpu}, and the
...
contribution of the work, and the three measurements of~\autoref{sec:evaluation}
converge on it~(\autoref{subsec:eval_discussion}). The latency evaluation
of~\autoref{subsec:latency_eval}
...
per-stage decomposition of~\autoref{subsec:latency_breakdown}
located that cost almost entirely in the analytical solve,
which alone accounts for \SI{89.7}{\percent} of per-frame compute,
with detection and filtering nearly free by comparison --- so that the system's
responsiveness rests on a single, well-understood stage. The resource profile
of~\autoref{subsec:hardware_util} confirmed the cost is also modest: the streamer and
the Unity application together draw only about a third of the machine's processor and
hold a flat memory footprint, leaving on average more than \SI{40}{\percent} of a
commodity \acrshort{cpu} idle. The
...
evaluation rather than design: the system was characterised for the latency and the cost
of the pose it delivers~(\autoref{sec:evaluation}), but the accuracy of that pose was
never validated. No benchmark figure quantifies how closely the streamed \acrshort{mano}
...
The most immediate is the accuracy evaluation that the performance study
of~\autoref{sec:evaluation} did not attempt. Benchmarking
...
interactions would complement those objective measures with evidence on the experience
itself.
...
reaching toward the learned ceiling. Any such addition, like any later effort to cut
latency or to track more than the present two hands, would fall on the analytical solve,
which the decomposition of~\autoref{subsec:latency_breakdown} identified as nearly the
whole of per-frame compute and thus the only stage where such work would tell.
Fusing a second camera or an optional depth sensor
