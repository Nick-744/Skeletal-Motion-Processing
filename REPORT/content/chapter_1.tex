...
from a standard colour camera, outputting 21 three-dimensional landmarks
per hand at around 30 frames per second on a commodity machine.
...
\cite{romero2017embodiedhands} is exactly this. It represents hand pose
...
were learned from real hand scans. A pose in MANO space can drive a 3-D mesh,
...
A hand tracking and 3-D coordinate module (\texttt{hand\_engine},
...
\cite{zhou2020monocular}, producing axis-angle joint rotations that are invariant
...
end-to-end latency of \SI{30.68}{\ms} over more than 20\,000 frames.
...
an editable city environment, a spring-physics face deformer, and a 2-D/3-D
painting tool with point-cloud export.

\subsection{Thesis Structure}\label{subsec:structure}

\Cref{sec:background} covers the theoretical background.
\Cref{sec:stateoftheart} reviews related work.
\Cref{sec:architecture} describes the overall system design.
\Cref{sec:tracking} details the hand tracking pipeline.
\Cref{sec:mano} explains the MANO fitting and IK solving.
\Cref{sec:unity} covers the Unity integration and the four scenes.
\Cref{sec:evaluation} presents the latency results.
\Cref{sec:reconstruction} describes the 3-D surface reconstruction pipeline.
\Cref{sec:conclusion} concludes with limitations and future directions.
