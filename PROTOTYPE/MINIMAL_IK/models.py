...
class KinematicModel():
  ...
  def set_params(self,
    ...
    shape:    np.ndarray = None) -> tuple[np.ndarray, np.ndarray]:
    '''
    Set model parameters and get the mesh. Do not set "pose_abs" and "pose_pca" at the same time.
    ...
    if   pose_abs is not None: self.pose = pose_abs
  ...
  def rodrigues(self, r: np.ndarray) -> np.ndarray:
    '''
    Rodrigues' rotation formula that turns axis-angle
    vector into rotation matrix in a batch-ed manner.
    ...
    m       = np.dstack([
      z_stick, -r_hat[:, 0, 2],  r_hat[:, 0, 1], r_hat[:, 0, 2],
      z_stick, -r_hat[:, 0, 0], -r_hat[:, 0, 1], r_hat[:, 0, 0],
      z_stick]
    ).reshape([-1, 3, 3])
  ...
  def pack(self, x: np.ndarray) -> np.ndarray:
    '''
    Append zero matrices of shape [4, 3] to vectors of [4, 1] shape in a batched manner.
  ...
  def save_obj(self, path: str) -> None:
    ...
    with open(path, 'w') as fp:
      for v in self.verts:     fp.write('v %f %f %f\n' % (v[0], v[1], v[2]))
      for f in self.faces + 1: fp.write('f %d %d %d\n' % (f[0], f[1], f[2]))
...
class KinematicPCAWrapper():
  ''' A wrapper for "KinematicModel" to be compatible to the solver. '''
  
  def __init__(self, core: KinematicModel, n_pose: int = 12):
    '''
    Parameters
    ----------
    core   : KinematicModel
  ...
  def run(self, params: np.ndarray) -> np.ndarray:
    ...
    return self.core.set_params(pose_glb = pose_glb, pose_pca = pose_pca, shape = shape)[1];

  def decode(self, params: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
